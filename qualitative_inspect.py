"""
Side-by-side qualitative comparison of NPO baseline vs NPO 4-bit quantized
...
MODELS = [
    ("NPO baseline", NPO_CKPT, {}),
    ("NPO 4-bit",    NPO_CKPT, {"quantization_config": BitsAndBytesConfig(load_in_4bit=True)}),
]
...
def load_model(name, ckpt, extra_kwargs):
    print(f"\nLoading {name} …")
...
def main():
    ...
    generations = {}
    for name, ckpt, kwargs in MODELS:
        model, tokenizer = load_model(name, ckpt, kwargs)
        gens = []
